...
/// If there are multiple ones with the same type all of them are returned.
  ...
  const std::vector<VRInputDevice*>& getInputDevices();
	...
	/// Provides access to pointers to input devices based on the type of the device,
	/// If no device of the requested type is found an empty vector is returned.
	/// If there are multiple ones with the same type all of them are returned.
	/// e.g. std::vector<VROpenVRInputDevice*> devices = _main->getDisplayNodesByType<VROpenVRInputDevice>();

	template <class T>
	std::vector<T*> getInputDeviceByType()
	{
		std::vector<T*> vec;
		for (int i = 0; i < _inputDevices.size(); i++)
		{
			T* tmp = dynamic_cast<T*>(_inputDevices[i]);
			if (tmp != nullptr)
			{
				vec.push_back(tmp);
			}
